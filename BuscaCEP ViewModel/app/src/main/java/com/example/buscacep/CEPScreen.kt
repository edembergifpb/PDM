...
@Composable
fun CEPScreen(viewModel: EnderecoViewModel = viewModel()) {
    ...
    Column(modifier = Modifier.padding(top = 60.dp), verticalArrangement = Arrangement.Center){
        ...
        when(state) {
            is CepUIState.Idle -> Text(text = "Digite um CEP")
            is CepUIState.Loading -> {
                Text(text = "Carregando...")
                //RotatingHourglass(imageVector = Icons.Filled.HourglassEmpty)
            }
